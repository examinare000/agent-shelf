...
from shelf.digests import DIGEST_SCHEMA, build_digest_prompt, parse_digest
...
from shelf.ports import AnswerBackend, FileSummary, NotebookCard, RetrievedChunk, RouteTarget
...
class ShelfService:
    def __init__(
        ...
        *,
        ...
        route_top_n: int = 1,
        route_fallback: str = "",
        digest_max_notes: int = 5,
        digest_input_max_chars: int = 4000,
        ...
        # digests.build_digest_prompt の切詰めしきい値へ配線するための保持値。
        # 既定 4000 は digests.DIGEST_INPUT_MAX_CHARS のローカル既定値と同値にして
        # 矛盾を避ける（digest_max_notes と同じ流儀）。
        self._digest_input_max_chars = digest_input_max_chars
    ...
    def digest(self, notebook: str, doc_id: str | None = None, *, force: bool = False) -> dict:
        ...
        for doc in targets:
            outcome = self._digest_one(notebook, doc, persona, backend_name, force=force)
            if outcome == "skipped":
    ...
    def _digest_one(
        ...
        *,
        ...
        """1資料分の学びノート生成。戻り値は "generated"/"skipped"/それ以外(エラー文言)。

        失敗（ok=False・parse失敗・例外・ファイル読取不可）は1資料で全体を止めず、
        呼び出し元 digest() が errors に記録して次の資料へ継続する
        ...
        if not force:
            ...
            if existing_notes and all(
                n["source_hash"] == content_hash for n in existing_notes
            ):
                return "skipped"
        ...
        prompt = build_digest_prompt(
            ...
            persona=persona,
            title=doc.get("title"),
            max_chars=self._digest_input_max_chars,
        )
        backend = self._backend_factory(backend_name)
        try:
            raw = backend.answer(
                prompt, workdir=self._corpus_dir / notebook, schema=DIGEST_SCHEMA
            )
        except Exception:  # noqa: BLE001 - 1資料の例外で digest 全体を止めない
            return "学びノート生成中に例外が発生しました"

        if not raw.ok:
            return raw.error or "学びノート生成に失敗しました"

        notes = parse_digest(raw.text, max_notes=self._digest_max_notes)
        if not notes:
            return "学びノートの解析に失敗しました"
